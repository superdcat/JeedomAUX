...
| **Code d'erreur du PROTOCOLE** ⚠️ | entier **signé** little-endian sur 2 octets en `0x22` de la **réponse**. `0` = pas d'erreur. Ne pas le lire ailleurs, ne pas le supposer non signé. ⚠️⚠️ **Ce champ dit si l'ÉCHANGE RÉSEAU a réussi — JAMAIS si le climatiseur est en défaut.** Il s'appelait « code d'erreur appareil » ici jusqu'à l'UC03 du domaine `post-mvp/04` : appellation corrigée parce qu'elle laissait croire qu'un canal de diagnostic d'appareil existait déjà en LAN. Il n'en existe **aucun**, sur aucun des deux transports — cf. `smartclim-transport-aux-home.md` § 4.5 |
